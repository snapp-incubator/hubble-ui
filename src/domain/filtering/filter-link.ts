...
export const filterLinkByEntry = (l: Link, e: FilterEntry): boolean => {
  ...
  switch (e.kind) {
    ...
    case FilterKind.Port: {
      // NOTE: A link only carries its destination port, so the port filter
      // matches regardless of filter direction
      const portMatch = l.destinationPort?.toString() === e.query;
      if (!portMatch && e.negative) return true;
